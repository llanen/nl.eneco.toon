...
class ToonOAuth2Client extends OAuth2Client {
   ...
   */
	async onGetTokenByCode({ code }) {
		...
		params.append('tenant_id', Homey.env.hasOwnProperty('TOON_TENANT_ID') ? Homey.env.TOON_TENANT_ID : 'eneco');
   ...
   */
	onHandleAuthorizationURL({ scopes, state } = {}) {
		...
		const query = {
			...
			// issuer: 'identity.toon.eu', // TODO: feature flag for new authorization flow
			response_type: 'code',
