...
class ToonOAuth2Client extends OAuth2Client {
	...
	/**
   * Method that exchanges a code for a token with the ToonAPI. Important is that the redirect_uri property contains a
   ...
   */
	...
	/**
   * Method that handles the creation of the authorization url. The ToonAPI expects a tenant_id property to be added
   * as query parameter.
   * @param scopes
   * @param state
   * @returns {string}
   ...
   */
	...
	/**
   * Method that checks if a webhook subscription is already registered, if not it will register a new subscription
   * with the ToonAPI.
   * @param {string} id - agreementId
   * @returns {Promise<*>}
   */
	async registerWebhookSubscription({ id, homeyId }) {
		this.log('registerWebhookSubscription()');
		const registeredSubscriptions = await this.getRegisteredWebhookSubscriptions({ id });
		if (registeredSubscriptions.length > 0) {
			this.log('registerWebhookSubscription() -> already registered');
	...
	/**
   * Method that checks if a webhook subscription is registered, in that case it will request to end the subscription
   * with the ToonAPI.
   * @param {string} id - agreementId
   * @returns {Promise<*>}
   */
	async unregisterWebhookSubscription({ id }) {
		...
		const registeredSubscriptions = await this.getRegisteredWebhookSubscriptions({ id });
		if (registeredSubscriptions.length === 0) {
			this.log('unregisterWebhookSubscription() -> no webhooks to unregister');
			return Promise.resolve();
		}
		return this.delete({
   ...
   */
	...
	/**
   * Method that fetches a status object from the ToonAPI containing all necessary device/capability information.
   * @param {string} id - agreementId
   * @returns {Promise<*>}
   */
	async getStatus({ id }) {
	...
	/**
   * Method that updates the thermostat state with the ToonAPI. Used for changing target temperature and temperature
   * states.
   * @param {string} id - agreementId
